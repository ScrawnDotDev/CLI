...
func writeEnvFile(cfg Config) error {
	content := strings.Join([]string{
		...
		fmt.Sprintf("LEMON_SQUEEZY_API_KEY=%s", quoteEnvValue(cfg.LemonSqueezyAPIKey)),
		fmt.Sprintf("LEMON_SQUEEZY_STORE_ID=%s", quoteEnvValue(cfg.LemonSqueezyStoreID)),
		fmt.Sprintf("LEMON_SQUEEZY_VARIANT_ID=%s", quoteEnvValue(cfg.LemonSqueezyVariantID)),
		fmt.Sprintf("LEMON_SQUEEZY_WEBHOOK_SECRET=%s", quoteEnvValue(cfg.LemonSqueezyWebhookSecret)),
		"TEST_API_KEY=",
